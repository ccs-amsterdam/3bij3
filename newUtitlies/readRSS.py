...
def readFeed(rssUrl, publisher, dbCursor,dbConnection):
    ...
    for article in newsFeed.entries:
        ...
        if(cleanUrl in remainingUrls):
            ...
            body,image = readBody(article.link)

            sql = "INSERT INTO articles(title, teaser, text, publisher, url, date, imageUrl,imageFilename) VALUES (%s, %s, %s,%s,%s, %s,%s,%s)"
            values = (article.title, "testTeaser", body, publisher, cleanUrl, makeDatetime(article.published), image,"")
...
def imageProcess(dbCursor, dbConnection):
    ...
    for image in images:
        ...
        # CHANGE THIS WHEN DEPLOYED TO REMOTE SERVER
        newFilename = "/home/stuart/newsflow/app/static/images/thumb_{}.{}".format(image[0],ext)

        # newFilename = "/home/newsrec/public_html/static/images/thumb_{}.{}".format(image[0],ext)
...
def main():
    ...
    for x in range(1, int(config["config"]["feedCount"]) + 1):
        ...
        readFeed(config[feedString]["url"],config[feedString]["publisher"], dbCursor, dbConnection)
